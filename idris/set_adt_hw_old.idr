...
--f is function that returns true when a value in the list is equal to a given value

--notAMember gives true if a value v is not a member of a list and
  --false if v is a member
notAMember: (eq a) => (v: a) -> list a -> bool
notAMember v nil = true
...
turnToBool: a -> bool
turnToBool ass(h) = true
turnToBool ass(f) = false

list_remove: (eq a) => (v: a) -> (l: list a) -> list a
list_remove v nil = nil
list_remove v l = filter turnToBool l
...
--toNat takes a value a and turns it to nat
toNat: a -> nat
toNat v = (S O)


--makeNat turns a list a into a list nat
makeNat: list a -> list nat
makeNat nil = nil
makeNat l = map toNat l


--newsum takes the elements in a list and adds them together
newsum: list nat -> nat
newsum nil = O
newsum l = list.foldr add O (makeNat l)

--set_cardinality emptySet = O
--set_cardinality (mkSet l) = newsum l
...
isTrue: (eq a) => a -> bool
--isTrue member = member


--boolList takes a list a and turns it into list bool

boolList: (eq a) => list a -> list bool
boolList nil = nil
boolList l = map isTrue l


--isHere takes a list bool and returns bool

isHere:(eq a) => list bool -> bool
isHere nil = false
isHere boolList = true




--set_member v (mkSet l) = isHere l
...
takeout: list a -> list a -> list a

--combine combines 2 sets into a single set



set_union (mkSet la) (mkSet lb) = nil
...
trueOrFalse: list a -> bool
trueOrFalse nil = false


set_forall p (mkSet l) = trueOrFalse (l)
...
--falseOrTrue takes a list bool and turns it into a true or false value


set_exists p (mkSet l) = true
...
--pairelt takes element h and list b and returns list of all pairs,
...
--given a list of a and list of b, return a list of all a b pairs
...
--instance (eq a) => eq (set a) where
 -- eql (mkSet l1) (mkSet l2) = eql_set (mkSet l1) (mkSet l2)
-------------------------------------
...
--powerset hint:powerset= set of sets,  powerset of emptyset is nil, powerset of 1 is set of
  --1 and emptyset, start from the bottom and build up 
  
--need testcases for main functions only, not test cases
